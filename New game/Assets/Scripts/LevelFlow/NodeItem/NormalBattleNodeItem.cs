...
public class NormalBattleNodeItem : BaseNodeItem
{
    ...
    {
        base.OnDestroy();
        EventCenter.Instance.RemoveEventListener<string>(E_EventType.Battle_NormalBattleWin, OnBattleWin);
    }

    protected override void OnNodeClick()
    {
        base.OnNodeClick();
        // 将当前节点ID传递给战斗模块（战斗模块需记录当前战斗的节点ID）
        EventCenter.Instance.EventTrigger(E_EventType.Battle_LoadNormalBattle, nodeId);
    }
